...
| Record number | **Context-anchored** — only taken after a label: `MRN`, `Hospital No/Number`, `Case No/Number`, `Record No/Number`, `Unit No/Number`, `Chart No/Number`, `Patient No/Number`, `Patient ID`, the UK register numbers (`GMC`, `NMC`, `HCPC`, `GPhC`), and the Australian identifiers `Medicare No/Number`, `Medicare Card No/Number`, `UR No/Number` and `Provider No/Number`. The label may carry a parenthetical gloss (`Hospital No (MRN): 4471982`) and the value may be grouped (`33-201-45`) or carry a trailing check letter (`2934 5671 0`, `2481726A`). A bare digit run in clinical text is far more likely to be a lab value |
...
| Address line | A labelled `Address:` line is taken **whole**. So is an **unlabelled letterhead street address** — house number, street name, a street-type word (`Street`, `Rd`, `Ave`, `Lane`, `Court`, …) and an optional suburb / AU state / postcode — when it sits in the first or last six lines (`LETTERHEAD_ADDRESS`). Taking the whole span is what stops a street address being left half-redacted, and it still lets layer 2 stay strict about bare place names |
