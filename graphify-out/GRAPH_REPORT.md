...
## Corpus Check
- 159 files · ~123,567 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2202 nodes · 4254 edges · 143 communities (121 shown, 22 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 95 edges (avg confidence: 0.61)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `776b1650`
...
Cohesion: 0.13
Nodes (26): DeidentifiedNote, deidentify_note(), leaked_values(), Run the real CareScribe de-identifier over a synthetic note. The fine-tune must…, De-identify one rendered+identified synthetic note., Injected identifier values that de-id did NOT remove from the text. A non-empty…, build_manifest(), _carescribe_sha() (+18 more)

### Community 3 - "schema.py"
Cohesion: 0.07
Nodes (53): BaseModel, Enum, field_validator, _blank_for(), expand(), Random, Turn vignettes into `EncounterFacts` instances with a seeded RNG. `expand`…, Build one `EncounterFacts` from a vignette. With ``gap_probability`` > 0, each… (+45 more)

### Community 4 - "batch.py"
Cohesion: 0.12
Nodes (20): BatchError, _default_output_dir(), list_folder(), Path, RuntimeError, Batch input and approved-output handling. The single module in CareScribe that…, Return the supported documents in ``folder``, sorted by name. Non-recursive on…, The raw bytes behind an upload or a path, without copying it to disk. (+12 more)
...
Cohesion: 0.08
Nodes (45): AppTest, Synthetic test data for the de-identification regression suite. EVERYTHING HERE…, Fully Fabricated Test Data, Synthetic Discharge Summary Fixture, Line-break-split Name Case, Precision Cases In Fixture, Recall Cases In Fixture, analysed_batch() (+37 more)

### Community 9 - "render_generation_panel"
Cohesion: 0.15
Nodes (13): _draft_state(), _model_card_path(), Path, Which backend will be used, and the fix if none is available., Shown instead of an empty panel when no model is available yet. An empty…, Option B. Ollama does the fetching; the request goes to loopback., A concrete "it works", rather than asking the clinician to trust a flag., Generate, refine, re-identify and export — for one approved document. Two… (+5 more)
...
Cohesion: 0.12
Nodes (22): available_forms(), (form_id, title) pairs — bundled forms first, then clinic-uploaded ones., fill_parsed_template(), Fill an in-memory copy of ``original_docx`` from ``spec``'s anchors. A thin…, _anchors(), _build_synthetic(), _merge_full_width(), fixture (+14 more)
...
Cohesion: 0.10
Nodes (34): _model_card_dialog(), privacy_indicator(), A persistent, honest statement of where data goes. It must change when cloud…, Name the generation model with a readable label; the card opens in a dialog…, _render_generation_model(), render_sidebar(), chip(), detection_layer() (+26 more)

### Community 27 - "main"
Cohesion: 0.21
Nodes (13): documents(), ingest_sources(), main(), _pipeline_step(), _privacy_state(), A missing model must stop loudly, never fall back to fetching one., The 0-based step the reviewer stands on, for components.step_tracker()., Extract text from uploads/paths into session state. (+5 more)
...
Cohesion: 0.12
Nodes (20): build_messages(), Care note templates. Every template shares the same hard rule: the input is…, Return ``(system, user)`` for a template label and de-identified document., FormType, The output form a training example asks the model to fill., GgufCompleter, Adapter over a local GGUF via llama-cpp-python. Greedy, temperature 0., build_messages() (+12 more)

### Community 37 - "canonical_person_key"
Cohesion: 0.18
Nodes (12): _collapse_person_identities(), True for a person row whose role is known (patient / relative / clinician)., Collapse every written form of one person onto a single entity row.…, _specific_person_type(), canonical_person_key(), keys_are_compatible(), A stable identity key for one person: full given name plus surname. This…, True if two canonical keys can denote the same person. Exact match, or one side… (+4 more)
...
Cohesion: 0.17
Nodes (13): cache_resource, load_detection_engine(), Load the NER model once per session, not once per rerun. Streamlit re-runs the…, engine_status(), get_analyzer(), get_gliner(), gliner_spans(), Layer 3: GLiNER detections, or an empty list when it isn't installed. (+5 more)
...
### Community 48 - "test_clinical_forms_generate.py"
Cohesion: 0.23
...
Cohesion: 0.11
Nodes (30): build_target(), The ideal filled form for ``facts`` — deterministic, fact-placed only., check_faithfulness(), check_format(), check_placeholders(), check_residual(), _field_is_empty(), _flatten_strings() (+22 more)
...
Cohesion: 0.07
Nodes (19): Stands in for a model so the egress test does not need one installed., StubBackend, _cloud_off(), _fresh_generation_status_cache(), mapping_module(), fixture, First-run generation setup: never an empty panel, and the egress line held. The…, The one outbound path must not be reachable from the de-id flow. (+11 more)
...
### Community 102 - "build_dataset.py"
Cohesion: 0.12
Nodes (27): build(), _fallback_inject(), _load_datagen_config(), main(), Path, End-to-end: sampled encounters -> validated SFT pairs + manifest. python -m…, Fill ``[[TOKEN]]`` slots with simple fake values. Used only until…, Return ``{"pairs": [...], "kept": k, "dropped": d, "reasons": {...}}``. (+19 more)

### Community 103 - "Initials-only patient reference (e.g. M.A.R.)"
Cohesion: 0.40
...
### Community 104 - "Cardiology Discharge Summary (doc02)"
Cohesion: 0.20
...
Cohesion: 0.09
Nodes (35): analyze_document(), load_documents(), Extract text from uploads or paths. Returns ``(documents, errors)``. One…, Run the de-identification layers over one document, in place., Findings from the safety sweep, minus the ones the reviewer has cleared. A…, Write approved de-identified text to the output folder. Re-runs the safety…, sweep(), write_approved() (+27 more)

### Community 111 - "test_mapping.py"
Cohesion: 0.09
...
### Community 112 - "sample_encounters"
Cohesion: 0.21
Nodes (10): Yield ``n`` `EncounterFacts`, deterministic for a given ``seed``., sample_encounters(), test_make_pair_uses_real_prompts_and_appends_the_target(), parametrize, The sampler must be deterministic, valid, and honest about gaps., test_every_vignette_expands_to_valid_facts(), test_gap_probability_one_gaps_everything_gappable(), test_gap_probability_zero_produces_no_gaps() (+2 more)
...
### Community 116 - "render_clinical_form_panel"
Cohesion: 0.22
...
### Community 120 - "GP Referral Letter (doc05)"
Cohesion: 0.25
...
Cohesion: 0.12
Nodes (20): build_map(), expand_facility_variants(), Issue, normalise_action(), In-memory PII <-> placeholder mapping. This module is deliberately pure: it…, Return the full organisation name plus short forms. "St. Aidan's General…, All strings that redact to a placeholder, plus collisions worth flagging., Expand entities into every surface form that should be redacted. Entity values… (+12 more)
...
### Community 131 - "write_approved_docx"
Cohesion: 0.29
...
### Community 135 - "resolve_model_path"
Cohesion: 0.50
...
### Community 142 - "stress_corpus/README.md"
Cohesion: 0.18
...
### Community 143 - "stress_report.py"
Cohesion: 0.67
...
- **What is the exact relationship between `stress_corpus/README.md` and `Mental Health Act Assessment Record (doc10)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `FormType` connect `FormType` to `test_assemble_pipeline.py`, `schema.py`, `build_dataset.py`, `EncounterFacts`, `test_train_and_grammar.py`, `validators.py`, `run_eval.py`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Why does `EncounterFacts` connect `EncounterFacts` to `test_assemble_pipeline.py`, `schema.py`, `FormType`, `build_dataset.py`, `sample_encounters`, `validators.py`, `run_eval.py`?**
