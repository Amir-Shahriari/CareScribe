...
## Summary
- 2328 nodes · 4489 edges · 150 communities (125 shown, 25 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 97 edges (avg confidence: 0.63)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `940470d4`
...
Cohesion: 0.10
Nodes (22): finalise(), Re-identify a draft locally and refuse to hand back a leaky document. Returns…, check_placeholder_integrity(), Compare a draft's bracketed tokens against the placeholders it should use. An…, Local re-identification of a generated draft. Returns ``(text, unresolved)``.…, reidentify_document(), Local generation: the privacy contract, placeholder integrity, and the gate. No…, Between [MRN_1] and [MRN_2], refusing is the only safe answer. (+14 more)
...
Cohesion: 0.15
Nodes (25): document_flags(), entity_confirmed(), entity_frame(), flag_dismissals(), One click to approve every document that comes back clean. Each is re-run…, Re-derive the preview and map from an edited entity list., Redact the original .docx using the map the reviewer just approved. Detection…, Offer the redacted .docx, but only once it has cleared the sweep. (+17 more)
...
Cohesion: 0.11
Nodes (29): Name the generation model with a readable label; the card opens in a dialog…, _render_generation_model(), render_sidebar(), chip(), detection_layer(), _esc(), hero(), icon() (+21 more)
...
### Community 29 - "app.py"
Cohesion: 0.16
Nodes (18): current(), documents(), ingest_sources(), main(), _model_card_dialog(), _pipeline_step(), CareScribe — local, privacy-preserving de-identification and review. Run with:…, A missing model must stop loudly, never fall back to fetching one. (+10 more)
...
Cohesion: 0.10
Nodes (31): _extract_docx(), _extract_pdf(), extract_text(), _extract_txt(), IngestError, Any, RuntimeError, Text extraction for uploaded documents (PDF / DOCX / TXT). Nothing here writes… (+23 more)
...
Cohesion: 0.17
Nodes (10): BackendError, LocalGGUFBackend, True if the runtime and a model file are both present., Raised when a backend cannot be used, with the fix in the message., Shared message for a completion cut off by the token/context budget. A half-…, CPU-only generation from a bundled GGUF via ``llama-cpp-python``. The model is…, _truncation_error(), RuntimeError (+2 more)
...
Cohesion: 0.07
Nodes (27): parametrize, Regression suite for the layered de-identification pipeline. Two guarantees,…, With REDACT_INPROSE_DATES False, a procedure date is clinical, not identity., DOB and admission/discharge dates carry identity, so they go., spaCy labels "ECG" and "NSTEMI" as organisations; the filter must drop them., One real value, one placeholder — the whole point of the mapping., The map is a return value the caller holds; nothing module-level keeps it., The NHS number must not survive under any spacing or hyphenation. (+19 more)
...
Cohesion: 0.11
Nodes (23): Pick a backend. Returns ``(kind, backend, label)``. ``prefer`` lets the UI…, select_backend(), _backend_with_fake_model(), _FakeModel, _raising_stream(), The happy path must keep working: finish_reason 'stop' yields the text with no…, The streaming path gets finish_reason only on its final, contentless chunk…, Mimics llama-cpp-python: create_chat_completion() returns a lazy generator, and… (+15 more)
...
### Community 103 - "render_clinical_form_panel"
Cohesion: 0.18
...
### Community 142 - "ClinicalFormError"
Cohesion: 0.67
Nodes (3): ClinicalFormError, RuntimeError, Raised when a clinical form can't be built or filled.
...
### Community 156 - "add_manual_entity"
Cohesion: 0.15
...
- **What is the exact relationship between `stress_corpus/README.md` and `Mental Health Act Assessment Record (doc10)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `inject()` connect `inject` to `run_eval.py`, `build_dataset.py`, `validators.py`?**
  _High betweenness centrality (0.044) - this node is a cross-community bridge._
- **Why does `deidentify()` connect `deidentify` to `test_app.py`, `deidentify.py`, `test_app_screens.py`, `test_pipeline_is_deterministic`, `test_pipeline_runs_without_ner`, `run_eval.py`, `test_deid_regressions.py`, `add_manual_entity`, `extract_text`, `test_stress_corpus.py`, `test_batch.py`, `mapping.py`, `redact`, `residual_scan`, `applog.py`, `test_deid_pipeline.py`, `NoEgress`, `test_generation_setup.py`, `analyze`, `_build_analyzer`?**
