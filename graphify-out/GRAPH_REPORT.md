...
## Corpus Check
- 182 files · ~144,963 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2489 nodes · 4707 edges · 149 communities (127 shown, 22 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 98 edges (avg confidence: 0.62)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `8044391e`
...
- BackendError
- render_prompt
- test_a_short_mapping_value_inside_an_ordinary_word_does_not_block
...
4. `FormType` - 43 edges
5. `Document` - 42 edges
6. `residual_scan()` - 33 edges
7. `generate_document()` - 27 edges
8. `extract_text()` - 26 edges
9. `sample_encounters()` - 26 edges
10. `generation_status()` - 23 edges
...
Cohesion: 0.10
Nodes (22): cache_data, generation_status(), _llama_runtime_available(), missing_reason(), Is generation usable right now, and if not, what should the user do? Kept…, One plain sentence on why generation is not available yet., Which generation backends are usable at this moment., Which backend would actually be used, matching the backend ladder. (+14 more)
...
Cohesion: 0.06
Nodes (49): approved_map(), document_has_text_boxes(), The reviewer-approved ``{literal: placeholder}`` map for the Word pass. This is…, True if a .docx holds text this redaction pass cannot reach., apply_redactions(), _delete_prefix(), extract_text(), has_unreachable_text() (+41 more)
...
Cohesion: 0.08
Nodes (26): finalise(), Re-identify a draft locally and refuse to hand back a leaky document. Returns…, check_placeholder_integrity(), Issue, One placeholder problem found in a generated draft., Compare a draft's bracketed tokens against the placeholders it should use. An…, Local re-identification of a generated draft. Returns ``(text, unresolved)``.…, reidentify_document() (+18 more)
...
Cohesion: 0.13
Nodes (16): _collapse_person_identities(), True for a person row whose role is known (patient / relative / clinician)., Collapse every written form of one person onto a single entity row.…, _specific_person_type(), expand_name_variants(), _initial_letters(), name_core(), Split a name into its parts with any leading honorific removed. "Mrs Margaret… (+8 more)
...
### Community 24 - "app.py"
Cohesion: 0.11
Nodes (36): current(), document_flags(), entity_confirmed(), entity_frame(), flag_dismissals(), _model_card_dialog(), _model_card_path(), Path (+28 more)
...
Cohesion: 0.09
Nodes (31): chip(), detection_layer(), empty_state(), _esc(), hero(), icon(), model_label(), privacy_line() (+23 more)
...
Cohesion: 0.09
Nodes (34): extract_text(), _extract_txt(), Any, Plain text, trying the encodings clinical exports actually use., Extract plain text from an uploaded pdf/docx/txt file. Raises…, Normalise a Streamlit UploadedFile (or a path string) to (name, bytes)., _read_bytes(), pandas (+26 more)
...
### Community 35 - "batch.py"
Cohesion: 0.06
Nodes (60): analyze_document(), approved_path(), BatchError, _default_output_dir(), list_folder(), load_documents(), Path, RuntimeError (+52 more)
...
Cohesion: 0.13
Nodes (21): build_map(), expand_facility_variants(), find_spans(), _form_pattern(), normalise_action(), normalise_type(), Pattern, In-memory PII <-> placeholder mapping. This module is deliberately pure: it… (+13 more)
...
Cohesion: 0.08
Nodes (29): add_manual_entity(), DeidentificationError, DeidResult, RuntimeError, Re-derive redacted text and the PHI map from a reviewer-edited table. Called…, Add an identifier the tools missed and immediately re-redact. The new value…, Raised when de-identification can't run at all., Everything the de-identification stage produces for one document. (+21 more)
...
### Community 45 - "RecordingBackend"
Cohesion: 0.16
Nodes (16): load_prompt(), The shared preamble — role, anti-fabrication rules, placeholder rules., Revise an existing draft against a follow-up instruction. Operates on the same…, Read one prompt file from ``carescribe/prompts``., refine_document(), system_prompt(), Captures exactly what generation handed the model., RecordingBackend (+8 more)
...
Cohesion: 0.09
Nodes (22): assert_no_residual_identifiers(), CareNoteError, generate_document(), RuntimeError, Refuse to send text the residual sweep still flags. :func:`assert_deidentified`…, Stream a drafted document from approved de-identified text. ``phi_values`` is…, Raised when care note generation can't proceed., The boundary check must not weaken a real leak: a short value standing alone as… (+14 more)
...
### Community 76 - "make_sample_docs.py"
Cohesion: 0.21
...
Cohesion: 0.15
Nodes (24): app_data_dir(), bundle_root(), ensure_dirs(), find_local_model(), is_frozen(), _model_search_dirs(), models_dir(), output_dir() (+16 more)
...
Cohesion: 0.13
Nodes (18): approved_docx_path(), Where the approved redacted .docx for ``name`` will be written., Redact the original .docx into the output folder, structure preserved. The same…, write_approved_docx(), _extract_docx(), _extract_pdf(), IngestError, normalise_line_endings() (+10 more)
...
### Community 109 - "safe_stem"
Cohesion: 0.24
Nodes (10): _as_docx(), The de-identified draft, refinement, re-identification, and exports., Opt-in, local-only substitution of placeholders back to real values., Render generated text to a .docx in memory — nothing touches disk., render_draft(), render_reidentification(), Reduce a filename to a safe output stem — no paths, no surprises., safe_stem() (+2 more)
...
Cohesion: 0.06
Nodes (39): assign_placeholders(), dedupe_entities(), _edit_distance(), Drop blank and duplicate entities, keeping first-seen order and casing.…, Attach a stable placeholder to each unique entity. A type with exactly one…, Levenshtein distance, short-circuiting once it exceeds ``cap``., Map a possibly-corrupted placeholder onto a known one. Returns the exact token…, Outcome of a re-identification pass. (+31 more)
...
### Community 143 - "main"
Cohesion: 0.13
Nodes (19): documents(), ingest_sources(), main(), PHI_KEYS (session-state PHI registry), _pipeline_step(), privacy_indicator(), _privacy_state(), A persistent, honest statement of where data goes. It must change when cloud… (+11 more)
...
Cohesion: 0.22
Nodes (9): Build the user prompt for one template with the source text embedded., render_prompt(), A case/hospital number has no home in Subjective/Objective/Assessment/Plan. The…, Same gap as the SOAP template, same fix: a "Re:" line names the patient but the…, test_an_unknown_template_is_refused(), test_the_clinic_letter_template_asks_for_administrative_identifiers(), test_the_custom_template_carries_the_clinicians_own_format(), test_the_custom_template_needs_instructions() (+1 more)

### Community 152 - "test_a_short_mapping_value_inside_an_ordinary_word_does_not_block"
Cohesion: 0.40
Nodes (5): parametrize, A 2-char mapping value must not refuse a clean draft just because those…, test_a_short_mapping_value_inside_an_ordinary_word_does_not_block(), test_each_template_renders_a_well_formed_prompt(), test_no_real_identifier_reaches_the_backend()
...
## Knowledge Gaps
- **287 isolated node(s):** `medgpt-finetune`, `merge_and_convert.sh script`, `build_dmg.sh script`, `build_macos.sh script`, `$schema` (+282 more)
...
- **Why does `deidentify()` connect `deidentify` to `build_dataset.py`, `test_letterhead_address_leak.py`, `test_crisis_lines_preserved.py`, `test_app.py`, `test_app_screens.py`, `test_deid_regressions.py`, `deidentify.py`, `extract_text`, `test_stress_corpus.py`, `batch.py`, `mapping.py`, `rebuild`, `test_combined_sources_generate_every_form_type_with_a_stub_backend`, `test_deid_pipeline.py`, `NoEgress`, `stress_report.py`, `test_generation_setup.py`, `analyze`, `applog.py`, `test_medicare_number_leak.py`, `test_labelled_id_leaks.py`?**
