...
## Corpus Check
- 156 files · ~121,331 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2162 nodes · 4174 edges · 137 communities (115 shown, 22 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 95 edges (avg confidence: 0.61)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `cfa617ed`
...
Cohesion: 0.29
Nodes (14): _biopsychosocial_spec(), FormField, FormSpec, _grid_fields(), HeaderField, _paragraph_texts(), Fill the three bundled APS clinical form templates from approved, de-identified…, The Biopsychosocial 'CLINICAL FORMULATION' table: a row-label × column-label… (+6 more)
...
Cohesion: 0.08
Nodes (38): approved_map(), document_has_text_boxes(), The reviewer-approved ``{literal: placeholder}`` map for the Word pass. This is…, Redact the original .docx into the output folder, structure preserved. The same…, True if a .docx holds text this redaction pass cannot reach., write_approved_docx(), extract_text(), has_unreachable_text() (+30 more)
...
Cohesion: 0.13
Nodes (21): available_forms(), (form_id, title) pairs — bundled forms first, then clinic-uploaded ones., fill_parsed_template(), Fill an in-memory copy of ``original_docx`` from ``spec``'s anchors. A thin…, test_no_field_key_collides_within_a_spec(), _anchors(), _build_synthetic(), _merge_full_width() (+13 more)
...
Cohesion: 0.15
Nodes (25): document_flags(), entity_confirmed(), entity_frame(), flag_dismissals(), Re-derive the preview and map from an edited entity list., Redact the original .docx using the map the reviewer just approved. Detection…, Offer the redacted .docx, but only once it has cleared the sweep., Candidate residuals for this document, recomputed from current text. (+17 more)

### Community 25 - "make_sample_docs.py"
Cohesion: 0.20
...
Cohesion: 0.23
Nodes (15): Name the generation model, and show its model card if one ships with it., _render_generation_model(), render_sidebar(), chip(), detection_layer(), empty_state(), _esc(), hero() (+7 more)

### Community 27 - "main"
Cohesion: 0.18
Nodes (14): current(), documents(), ingest_sources(), main(), _pipeline_step(), _privacy_state(), A missing model must stop loudly, never fall back to fetching one., The 0-based step the reviewer stands on, for components.step_tracker(). (+6 more)
...
Cohesion: 0.20
Nodes (10): _form_draft_key(), _invalidate_form_export(), Drop any previously re-identified/exportable content — called whenever the…, render_form_reidentification(), Bug: stale re-identified export race after refine/regenerate, Pure-logic pieces of the clinical-form UI: the session-state key used to key a…, test_form_draft_key_differs_by_form_or_selection(), test_form_draft_key_is_stable_for_the_same_selection() (+2 more)
...
Cohesion: 0.23
Nodes (12): Render a stream token by token so a slow local model looks alive., First-pass generation. The model receives de-identified text only., Follow-up instructions, on de-identified text only., render_form_refinement(), render_refinement(), _run_form_generation(), _run_generation(), _stream_into() (+4 more)
...
### Community 57 - "app.py"
Cohesion: 0.17
Nodes (18): _draft_state(), _form_draft_state(), _header_values_complete(), CareScribe — local, privacy-preserving de-identification and review. Run with:…, Which backend will be used, and the fix if none is available., Shown instead of an empty panel when no model is available yet. An empty…, Option B. Ollama does the fetching; the request goes to loopback., A concrete "it works", rather than asking the clinician to trust a flag. (+10 more)
...
### Community 96 - "render_form_draft"
Cohesion: 0.25
...
Cohesion: 0.14
Nodes (16): assert_deidentified(), assert_no_residual_identifiers(), CareNoteError, generate_care_note(), OllamaBackend, RuntimeError, Care note generation — local, on approved de-identified text only. The contract…, True only when ``needle`` occurs in ``haystack`` as a whole token run. Both are… (+8 more)
...
- **What is the exact relationship between `stress_corpus/README.md` and `Mental Health Act Assessment Record (doc10)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `FormType` connect `FormType` to `build_dataset.py`, `schema.py`, `EncounterFacts`, `test_train_and_grammar.py`, `run_eval.py`, `metrics.py`, `test_validators.py`?**
  _High betweenness centrality (0.061) - this node is a cross-community bridge._
- **Why does `Document` connect `Document` to `test_batch.py`, `batch.py`, `test_docx_roundtrip.py`, `test_app.py`, `test_generation.py`, `docx_redact.py`, `select_backend`, `render_draft`, `app.py`, `main`, `make_sample_docs.py`?**
